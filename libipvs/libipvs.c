 ...
 *
 * Version:     $Id: libipvs.c,v 1.4.2.1 2002/11/14 10:05:28 wensong Exp $
 *
...
int
ipvs_cmp_services(struct ip_vs_service_user *s1, struct ip_vs_service_user *s2)
{
	int r;
...
ipvs_sort_services(struct ip_vs_get_services *s, ipvs_service_cmp_t f)
{
	qsort(s->entrytable, s->num_services,
	      sizeof(struct ip_vs_service_user), (qsort_cmp_t)f);
}
...
int ipvs_cmp_dests(struct ip_vs_dest_user *d1, struct ip_vs_dest_user *d2)
{
	int r;

	r = ( ntohl(d1->addr) - ntohl(d2->addr) );
	if ( !r )
		r = ( ntohs(d1->port) - ntohs(d2->port) );
...
void ipvs_sort_dests(struct ip_vs_get_dests *d, ipvs_dest_cmp_t f)
{
	qsort(d->entrytable, d->num_dests,
	      sizeof(struct ip_vs_dest_user), (qsort_cmp_t)f);
}
